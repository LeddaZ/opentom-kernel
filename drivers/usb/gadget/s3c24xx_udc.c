 ...
 *
 * Modified to support full s3c24xx range for TomTom GO devices
 *		- by Ithamar R. Adema <ithamar.adema@tomtom.com>
 *
...
static struct completion *usb_enum_completion;

static const char driver_name[] = "s3c24xx_udc";
...
static struct usb_request *elfin_alloc_request(struct usb_ep *ep, unsigned);
...
static void elfin_free_buffer(struct usb_ep *ep, void *, dma_addr_t,
				unsigned);
static int elfin_queue(struct usb_ep *ep, struct usb_request *, unsigned);
...
static void udc_reinit(struct elfin_udc *dev)
{
	...
	for (i = 0; i < S3C24XX_MAX_ENDPOINTS; i++) {
...
static void done(struct elfin_ep *ep, struct elfin_request *req, int status)
{
	...
	if (status && status != -ESHUTDOWN)
		DPRINTK("complete %s req %p stat %d len %u/%u\n",
...
//	    || ep_maxpacket(ep) < le16_to_cpu(desc->wMaxPacketSize)
		...
		DPRINTK("IRA: ep->desc=%p, bEndPointAddress=%d/%d\n", ep->desc, desc->bEndpointAddress, ep->bEndpointAddress);
		DPRINTK("%s, bad ep or descriptor\n", __FUNCTION__);
		return -EINVAL;
	}
			 ...
			 unsigned gfp_flags)
{
	...
	DPRINTK("%s queue req %p, len %d buf %p\n", _ep->name, _req, _req->length,
	      _req->buf);
...
static void elfin_ep0_setup(struct elfin_udc *dev, u32 csr)
{
	...
	if (	usb_enum_completion != NULL &&
		ctrl.bRequestType == USB_DIR_OUT && /* from host to device */
		ctrl.bRequest == USB_REQ_SET_CONFIGURATION) {
		/* Host has enumerated the device; we now continue normally with boot process */
		complete(usb_enum_completion);
	}
}
...
static int elfin_udc_probe(struct device *_dev)
{
	...
	retval =
	    request_irq(IRQ_USBD, elfin_udc_irq, SA_INTERRUPT, driver_name,
			dev);
	if (retval != 0) {
		DPRINTK(KERN_ERR "%s: can't get irq %i, err %d\n", driver_name,
		      IRQ_USBD, retval);
		return -EBUSY;
	}
